      ...
      if (docsRes.data && docsRes.data.length > 0) {
        for (const doc of docsRes.data) {
          ...
          masterDocs.push({
            ...
            status: 'active',
            verification_status: doc.verification_status === 'VERIFIED' ? 'VERIFIED' : (doc.verification_status === 'REJECTED' ? 'REJECTED' : 'PENDING_VERIFICATION'),
      ...
      if (reqsRes.data && reqsRes.data.length > 0) {
        for (const req of reqsRes.data) {
          ...
          const docPayload: DocumentMaster = {
            ...
            classification: (matchingType?.default_classification as any) || 'restricted',
            status: 'active',
            verification_status: req.status === 'VERIFIED' ? 'VERIFIED' : (req.status === 'REJECTED' ? 'REJECTED' : 'PENDING_VERIFICATION'),
            current_version_id: versionId,
            current_version: curVer,
            versions: [curVer],
            version_count: 1,
