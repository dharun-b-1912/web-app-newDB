...
import { Sparkles, Send, Bot, User, HelpCircle, ShieldCheck } from 'lucide-react';
...
export const AiAssistantDrawer: React.FC<AiAssistantDrawerProps> = ({ isOpen, onClose }) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      sender: 'ai',
      text: `Hello! I am your WorkForceOS Intelligence Copilot powered by Google Gemini. How can I assist you with HR policies, department headcount analysis, or multi-tenant configuration for ${activeCompany?.legal_name}?`,
      time: 'Just now',
    },
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const aiResponseText = await askWorkForceCopilot(userMsgText, {
  ...
  return (
    <Drawer isOpen={isOpen} onClose={onClose} title="WorkForce Copilot Intelligence" size="md">
      <div className="flex flex-col h-[calc(100vh-140px)] justify-between space-y-4">
        ...
        <div className="space-y-3 overflow-y-auto pr-1 flex-1">
          {messages.map(m => (
            ...
            >
              {m.sender === 'ai' && (
                <div className="w-7 h-7 rounded-lg bg-[#07563D] text-white flex items-center justify-center shrink-0">
              ...
              <div
                className={`p-3 rounded-2xl max-w-[85%] space-y-1 ${
                  m.sender === 'user'
                    ? 'bg-[#07563D] text-white rounded-br-none'
        ...
        <div className="pt-2 border-t border-gray-100 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            <button
              onClick={() => setInput('What is our current department headcount breakdown?')}
              className="text-[11px] bg-emerald-50 text-[#07563D] hover:bg-emerald-100 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
            >
              Headcount breakdown
            </button>
            <button
              onClick={() => setInput('Explain the leave policy entitlements')}
              className="text-[11px] bg-emerald-50 text-[#07563D] hover:bg-emerald-100 px-2.5 py-1 rounded-lg transition-colors cursor-pointer"
            >
          ...
          <form onSubmit={handleSend} className="flex gap-2 pt-2">
            <input
              ...
              placeholder="Ask Copilot about HR policies, headcount..."
